...
1. **Perceive** — pulls logs from the requested source (paste / upload / Datadog / Grafana / New Relic).
2. **Plan & observe** — the agent calls five tools (`extract_entities`, `correlate_timeline`, `service_dependency_hints`, `search_logs`, `query_similar_incidents`) to inventory the telemetry, test its strongest hypothesis, and check for related past incidents.
...
The full reasoning trail is exposed in the API and rendered in the UI under **Agent reasoning trail** — click any tool call to inspect its raw JSON output. See [ARCHITECTURE.md](ARCHITECTURE.md) and [HACKATHON.md](HACKATHON.md) for the deep dive.

## Core features

| Feature | Description |
| --- | --- |
| **Multi-step agent with tool use** | Real think → act → observe → decide loop; reasoning trail visible in the UI |
